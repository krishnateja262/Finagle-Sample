...
public class SearchServiceImpl implements SearchService {
    ...
    public ArrayList<Product> searchSnapdeal(String searchString) throws UnirestException {
        ...
        Document document = Jsoup.parse(remoteService.snapdealSearch(searchString,20));
        Elements products = document.select("div.product_grid_row > div.product_grid_cont");
        products.forEach(product -> {
            ...
            productsModel.add(new Product(title, price, url, originalPrice, discount, vendor, productId,TitleSimilarity.similarityIndex(title.toLowerCase(),searchString.toLowerCase())));
        });
    ...
    public ArrayList<Product> searchFlipkartWebSite(String searchString) throws UnirestException {
        ArrayList<Product> productsModel = new ArrayList<>();
        Document document = Jsoup.parse(remoteService.flipkartSearch(searchString));
        Elements products = document.select("#products > div.old-grid > div.gd-row > div.gd-col");
        products.forEach(product -> {
            String title = product.select("div.pu-title > a").attr("title");
            String price = product.select("div.pu-final > span").text();
            String url = "http://www.flipkart.com"+product.select("div.pu-title > a").attr("href");
            String originalPrice = product.select("div.pu-discount > span.pu-old").text();
            String discount = product.select("div.pu-discount > span.pu-off-per").text();
            String productId = product.select("div.product-unit").attr("data-pid");
            String vendor = "flipkart";
            String outOfStock = product.select("a.pu-status").text().replaceAll(" ","");
            Product prod = new Product(title,price,url,originalPrice,discount,vendor,productId, TitleSimilarity.similarityIndex(title.toLowerCase(),searchString.toLowerCase()));
    ...
    public ArrayList<Product> searchAmazon(String searchString) throws UnirestException {
        ArrayList<Product> productsModel = new ArrayList<>();
        Document document = Jsoup.parse(remoteService.amazonSearch(searchString));
        ...
        products.forEach(product -> {
            ...
            String price = product.select("a.a-link-normal > span.a-size-base").text().replaceAll("\\u00a0", "");
            String originalPrice = product.select("span.a-text-strike").text().replaceAll("\\u00a0", "");
            String discount = product.select("div.a-row > span.a-color-price").text().replaceAll("\\u00a0", "");
            String vendor = "amazon";
            String productId = url.substring(url.indexOf("dp/")+3, url.indexOf("/", url.indexOf("dp/") + 3));
            productsModel.add(new Product(title,price,url,originalPrice,discount,vendor,productId,TitleSimilarity.similarityIndex(title.toLowerCase(),searchString.toLowerCase())));
        });
        return sortProducts(productsModel);
    }
    ...
    private ArrayList<Product> sortProducts(ArrayList<Product> products){
        Collections.sort(products, new Comparator<Product>() {
            ...
            public int compare(Product o1, Product o2) {
                return (int)((o2.getSimilarityIndex()-o1.getSimilarityIndex())*10);
            }
        });
        return products;
    }
